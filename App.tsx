...
import { ReadingProgress } from './animations/ReadingProgress';
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <motion.nav
        ...
        className="bg-white border-b border-slate-100 px-6 lg:px-12 py-5 flex items-center justify-between sticky top-0 z-40"
      >
        <div className="flex items-center gap-10">
          <div className="flex items-center gap-2">
            <motion.div
              whileHover={{ rotate: 10, scale: 1.1 }}
              className="w-8 h-8 bg-[#4c44d4] rounded flex items-center justify-center text-white font-black text-xs"
            >B</motion.div>
            <span className="font-display font-black text-lg tracking-tighter">BlogHub</span>
          </div>
          <div className="hidden lg:flex gap-8 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
            <a href="#" className="hover:text-[#4c44d4] transition-colors">Tools</a>
            <a href="#" className="hover:text-[#4c44d4] transition-colors">Practice</a>
            <a href="#" className="hover:text-[#4c44d4] transition-colors">Events</a>
            <a href="#" className="hover:text-[#4c44d4] transition-colors">Job Board</a>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.97 }}
            className="px-5 py-2 bg-[#4c44d4] text-white rounded-lg text-xs font-bold hover:bg-[#3b35a8] transition-colors shadow-lg shadow-blue-200"
          >
            Profile
          </motion.button>
        </div>
      ...
      {/* Hero */}
      <header className="bg-white py-16 lg:py-24 text-center px-6 border-b border-slate-50 relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none opacity-[0.04]">
          <div className="absolute -top-24 -left-24 w-96 h-96 rounded-full bg-[#4c44d4] blur-3xl" />
          <div className="absolute -bottom-24 -right-24 w-96 h-96 rounded-full bg-[#4c44d4] blur-3xl" />
        </div>
        <FadeIn className="relative" delay={0.1}>
          <TextReveal
            text="BlogHub"
            className="font-display text-4xl lg:text-[52px] font-[900] text-[#1a1a1a] mb-4 tracking-tighter"
          />
        </FadeIn>
        <FadeIn delay={0.3}>
      ...
      <main className="max-w-[1280px] mx-auto w-full flex flex-col lg:flex-row gap-8 px-6 pb-20 mt-12">
        <aside className="w-full lg:w-[340px] shrink-0" role="complementary">
          ...
          <div className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
            {isLoading ? (
              ...
              filteredBlogs.map((blog, i) => (
                <AnimatedSection key={blog.id} delay={i * 0.05} y={20}>
                  <BlogCard 
                    blog={blog}
              ...
              <FadeIn className="p-8 text-center text-slate-400">
      ...
      <footer className="bg-[#111111] text-white py-16 px-12 mt-auto text-center md:text-left">
        <div className="max-w-[1280px] mx-auto">
          <div className="flex items-center justify-center md:justify-start gap-2 mb-4">
            <div className="w-8 h-8 bg-white text-black rounded flex items-center justify-center font-black">B</div>
            <span className="font-display font-black text-lg tracking-tighter">BlogHub</span>
          </div>
        ...
        {isCreating && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-2xl rounded-[32px] p-10 max-h-[90vh] overflow-y-auto shadow-2xl custom-scrollbar"
            >
      ...
      <AnimatePresence>
        {editingBlog && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-lg flex items-center justify-center p-4"
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 30 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
            ...
            >
               <div className="p-10 md:p-14 overflow-y-auto max-h-[85vh] custom-scrollbar">
                  ...
                  {updateError && (
                    <div className="mb-8 p-6 bg-red-50 border border-red-100 rounded-[24px] flex items-start gap-4">
        ...
        {deletingId && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-sm rounded-[32px] p-10 text-center shadow-2xl"
            >
